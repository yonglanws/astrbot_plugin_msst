...
- **队列与公平调度**：任务入队排队（容量 20），默认 2 路并发导出（需与宿主 `render.workers` 对齐）；
  聊天任务优先于剧本任务；同一用户每轮最多占用一个并发槽，防止刷屏；超时自动取消，失败自动重试（最多 2 次）。
  并发入队、重试与满队列拒绝均有测试覆盖
...
> 台词排版硬规则、可用动作/表情清单）。`/剧本生成` 按约 3 分钟短戏收束，禁止超长剧情；对话多少、
> 每句长短仍由角色人设自行决定，不额外规定条数或字数。台词排版另有程序化兜底（折叠连续换行、
> 按 26 字宽硬折行、超 3 行的 Talk 自动拆成同角色连续多条），只排版不删减台词，防止字幕溢出。
> 角色人设部分来自
